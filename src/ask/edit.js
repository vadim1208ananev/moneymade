...
import { getDataAskWidget, getTickerBySymbol, getUserData, getToken, getUserDomains, getProfiles  } from "./api"
/**
...
export default function Edit(props) {
	const { attributes: { domain, domain_arr, profile, profile_arr,ask_source, auth_result, ask_source_arr, ask_monitization_type, ask_source_code,
		 html_to_iframe,ask_title
	}, setAttributes } = props;
		...
		if (!window.step_domains) {
			...
			getUserData().then(res => {
				if (!res.status || !res.user_data || !res.prepare_data) {
					...
					setAttributes({ ask_source_arr: empty_sources.slice() }) //individ
					setAttributes({ ask_source: '' }) //individ					
				...
				getToken(raw).then(res => {
					...
					getUserDomains(accessToken).then(res => {
						...
						setAttributes({ ask_source_arr: existing_sources.slice() }) ////individ
			...
			if (!ask_source_arr && window.existing_sources) {
				setAttributes({ ask_source_arr: window.existing_sources }) ////individ
			}
	...
	function setAskSource(source, domain_first = false) {
		setAttributes({ ask_source: source });
		if (domain_first) {
		...
		getProfiles(domain_first).then(
			res => {
				...
				return getDataAskWidget(send_data)
			}
		).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ ask_source_code: final_code });
	...
	function setProfile(item) {
		...
		getDataAskWidget(send_data).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ ask_source_code: final_code });
	...
	function setAskSourceCode(code) { //extra capobility
		setAttributes({ ask_source_code: code });
	}
	...
	return (
		<div {...useBlockProps()}>
			<div class="block-widget">
				<div class="block-widget-title">Ask An Expert Widget</div>
				<div class="block-widget-body">
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							value={ask_source}
							onChange={(source) => { setAskSource(source) }}
