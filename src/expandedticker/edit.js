...
import { getDataExpandedTickerWidget, getTickerBySymbol, getUserData, getToken, getUserDomains, getProfiles } from "./api"
/**
...
export default function Edit(props) {
	const { attributes: { domain, domain_arr, profile, profile_arr, symbol_search, expandedticker_source, auth_result, expandedticker_source_arr, expandedticker_monitization_type, expandedticker_source_code, expandedticker_ticker,
		expandedticker_comparasion_ticker, html_to_iframe
	}, setAttributes } = props;
		...
		if (!window.step_domains) {
			getUserData().then(res => {
				if (!res.status || !res.user_data || !res.prepare_data) {
					...
					setAttributes({ expandedticker_source_arr: empty_sources.slice() }) //individ
					setAttributes({ expandedticker_source: '' }) //individ					
				...
				getToken(raw).then(res => {
					...
					getUserDomains(accessToken).then(res => {
						...
						setAttributes({ expandedticker_source_arr: existing_sources.slice() }) ////individ
			...
			if (!expandedticker_source_arr && window.existing_sources) {
				setAttributes({ expandedticker_source_arr: window.existing_sources }) ////individ
			}
	...
	function setExpandedTickerSource(source, domain_first = false) {
		setAttributes({ expandedticker_source: source });
		if (domain_first) {
	...
	function setExpandedTickerMonitizationType(type) {
		...
		getDataExpandedTickerWidget(send_data).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ expandedticker_source_code: final_code });
	...
	function setExpandedTickerSourceCode(code) { //extra capobility
		setAttributes({ expandedticker_source_code: code });
	}
	...
	function setTicker(ticker) {
		if (ticker.includes('abbr') && regSticker(ticker)) {
			setAttributes({ expandedticker_ticker: ticker });
			let send_data = {
				expandedticker_monitization_type: expandedticker_monitization_type,
				expandedticker_source: expandedticker_source,
	...
	function setComparasionTicker(ticker) {
		if (ticker.includes('abbr') && regSticker(ticker)) {
			setAttributes({ expandedticker_comparasion_ticker: ticker });
			let send_data = {
				expandedticker_monitization_type: expandedticker_monitization_type,
				expandedticker_source: expandedticker_source,
				expandedticker_ticker: regSticker(expandedticker_ticker),
	...
	function delTicker() {
		setAttributes({ expandedticker_ticker: '' });
		let send_data = {
			expandedticker_monitization_type: expandedticker_monitization_type,
			expandedticker_source: expandedticker_source,
	...
	function delComparasionTicker() {
		setAttributes({ expandedticker_comparasion_ticker: '' });
		let send_data = {
			expandedticker_monitization_type: expandedticker_monitization_type,
			expandedticker_source: expandedticker_source,
	...
	function setChecked(search) {
		setAttributes({ symbol_search: !symbol_search });
		let send_data = {
			expandedticker_monitization_type: expandedticker_monitization_type,
			expandedticker_source: expandedticker_source,
			expandedticker_comparasion_ticker: regSticker(expandedticker_comparasion_ticker),
	...
	return (
		<div {...useBlockProps()}>
			<div class="block-widget">
				<div class="block-widget-title">Expanded Ticker Widget</div>
				...
				<div class="block-widget-body">
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							onChange={(source) => { setExpandedTickerSource(source) }}
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							onChange={(type) => { setExpandedTickerMonitizationType(type) }}
							options={[
						...
						<div class="blockrb">
							<div>
								<RichText
									...
									placeholder={__(`Click ${autoConfigs[0].triggerPrefix} to find Comparasion Ticker`)}
								/>
							</div>
							<div>
								<Button
									...
									variant="secondary">
									Delete comparasion ticker</Button>
							</div>
						</div>
					</div>
					<hr></hr>
