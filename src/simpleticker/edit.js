...
import { getDataSimpleWidget, getTickerBySymbol, getUserData, getToken, getUserDomains, getProfiles } from "./api"
/**
...
export default function Edit(props) {
	const { attributes: { domain, domain_arr, profile, profile_arr, symbol_search, simple_source, auth_result, simple_source_arr, simple_monitization_type, simple_source_code, simple_ticker, arr_tick, html_to_iframe
	}, setAttributes } = props;
	useEffect(() => {
		if (!window.step_domains) {
			getUserData().then(res => {
				if (!res.status || !res.user_data || !res.prepare_data) {
					...
					setAttributes({ simple_source_arr: empty_sources.slice() }) //individ
					setAttributes({ simple_source: '' }) //individ					
				...
				getToken(raw).then(res => {
					...
					getUserDomains(accessToken).then(res => {
						...
						setAttributes({ simple_source_arr: existing_sources.slice() }) ////
			...
			if ((!simple_source_arr) && (window.existing_sources)) {
				setAttributes({ simple_source_arr: window.existing_sources }) ////individ		
			}
	...
	function setSimpleSource(source, domain_first = false) {
		setAttributes({ simple_source: source });
		if (domain_first) {
		...
		getProfiles(domain_first).then(
			res => {
				...
				return getDataSimpleWidget(send_data)
			}
		).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ simple_source_code: final_code });
	...
	function setProfile(item) {
		...
		getDataSimpleWidget(send_data).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ simple_source_code: final_code });
	...
	function setSimpleMonitizationType(type) {
		setAttributes({ simple_monitization_type: type });
		let send_data = {
	...
	function setSimpleSourceCode(code) { //extra capobility
		setAttributes({ simple_source_code: code });
	}
	...
	function getTicker(ticker) {
		if (ticker.includes('abbr') && regSticker(ticker)) {
			setAttributes({ simple_ticker: ticker });
			let send_data = {
				simple_monitization_type: simple_monitization_type,
				simple_source: simple_source,
	...
	return (
		<div {...useBlockProps()}>
			<div class="block-widget">
				<div class="block-widget-title">Simple Ticker Widget</div>
				<div class="block-widget-description">A returns chart for one publicly traded ticker, asset class or asset sub-category.</div>
				<div class="block-widget-body">
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							value={simple_source}
							onChange={(source) => { setSimpleSource(source) }}
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							onChange={(type) => { setSimpleMonitizationType(type) }}
							options={[
