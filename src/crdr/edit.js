...
import { getDataСrdrWidget, getTickerBySymbol, getUserData, getToken, getUserDomains, getProfiles } from "./api"
/**
...
export default function Edit(props) {
	const { attributes: { domain, domain_arr, profile, profile_arr, crdr_source, auth_result, crdr_source_arr, crdr_source_code,
		html_to_iframe, crdr_title
	}, setAttributes } = props;
		...
		if (!window.step_domains) {
			getUserData().then(res => {
				if (!res.status || !res.user_data || !res.prepare_data) {
					...
					setAttributes({ crdr_source_arr: empty_sources.slice() }) //individ
					setAttributes({ crdr_source: '' }) //individ					
				...
				getToken(raw).then(res => {
					...
					getUserDomains(accessToken).then(res => {
						...
						setAttributes({ crdr_source_arr: existing_sources.slice() }) ////individ
			...
			if (!crdr_source_arr && window.existing_sources) {
				setAttributes({ crdr_source_arr: window.existing_sources }) ////individ
			}
	...
	function setСrdrSource(source, domain_first = false) {
		setAttributes({ crdr_source: source });
		if (domain_first) {
		...
		getProfiles(domain_first).then(
			res => {
				...
				return getDataСrdrWidget(send_data)
			}
		).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ crdr_source_code: final_code });
				setAttributes({ html_to_iframe: html_to_iframe })
			}
		)
	}

	function setСrdrSourceCode(code) { //extra capobility
		setAttributes({ crdr_source_code: code });
	}

	function setTitle(title) {
		setAttributes({ crdr_title: title });
		let send_data = {
			crdr_title: title,
			crdr_source: crdr_source,
			profile: profile
		}
		getDataСrdrWidget(send_data).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ crdr_source_code: final_code });
	...
	return (
		<div {...useBlockProps()}>
			<div class="block-widget">
				<div class="block-widget-title">Compact Revenue Driver Widget</div>
				<div class="block-widget-body">
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							value={crdr_source}
							onChange={(source) => { setСrdrSource(source) }}
