...
import { getDataCompareplatformWidget, getTickerBySymbol, getUserData, getToken, getUserDomains, getPlatforms, getProfiles } from "./api"
/**
...
export default function Edit(props) {
	const { attributes: { domain, domain_arr, profile, profile_arr, compareplatform_source, auth_result, compareplatform_source_arr, compareplatform_source_code,
		html_to_iframe, compareplatform_title, platform1, platform2, platform3
	}, setAttributes } = props;
		...
		if (!window.step_domains) {
			getUserData().then(res => {
				if (!res.status || !res.user_data || !res.prepare_data) {
					...
					setAttributes({ compareplatform_source_arr: empty_sources.slice() }) //individ
					setAttributes({ compareplatform_source: '' }) //individ					
				...
				getToken(raw).then(res => {
					...
					getUserDomains(accessToken).then(res => {
						...
						setAttributes({ compareplatform_source_arr: existing_sources.slice() }) ////individ
			...
			if (!compareplatform_source_arr && window.existing_sources) {
				setAttributes({ compareplatform_source_arr: window.existing_sources }) ////individ
			}
	...
	function setCompareplatformSource(source, domain_first = false) {
		setAttributes({ compareplatform_source: source });
		if (domain_first) {
		...
		getProfiles(domain_first).then(
			res => {
				...
				return getDataCompareplatformWidget(send_data)
			}
		).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ compareplatform_source_code: final_code });
	...
	function setProfile(item) {
		...
		getDataCompareplatformWidget(send_data).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ compareplatform_source_code: final_code });
	...
	function setCompareplatformSourceCode(code) { //extra capobility
		setAttributes({ compareplatform_source_code: code });
	}
	...
	function setTitle(title) {
		setAttributes({ compareplatform_title: title });
		let send_data = {
	...
	return (
		<div {...useBlockProps()}>
			<div class="block-widget">
				<div class="block-widget-title">Compare Platforms Widget</div>
				<div class="block-widget-description">Widget which allow to compare 3 or less platforms</div>
				...
				<div class="block-widget-body">
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							onChange={(source) => { setCompareplatformSource(source) }}
					...
					<div class="blockrb">
						<div>
							<RichText
								...
								placeholder={__(`Click ${autoConfigs[0].triggerPrefix} to find Platform1`)}
							/>
						</div>
						<div>
							<Button
					...
					<div class="blockrb">
						<div>
							<RichText
								...
								placeholder={__(`Click ${autoConfigs[0].triggerPrefix} to find Platform2`)}
							/>
						</div>
						<div>
							<Button
					...
					<div class="blockrb">
						<div>
							<RichText
								...
								placeholder={__(`Click ${autoConfigs[0].triggerPrefix} to find Platform3`)}
							/>
						</div>
						<div>
							<Button
