...
import { getDataSliderestimatesWidget, getTickerBySymbol, getUserData, getToken, getUserDomains, getProfiles } from "./api"
/**
...
export default function Edit(props) {
	const { attributes: { domain, domain_arr, profile, profile_arr, sliderestimates_source, auth_result, sliderestimates_source_arr, sliderestimates_source_code,
		html_to_iframe, sliderestimates_title
	}, setAttributes } = props;
		...
		if (!window.step_domains) {
			...
			getUserData().then(res => {
				if (!res.status || !res.user_data || !res.prepare_data) {
					...
					setAttributes({ sliderestimates_source_arr: empty_sources.slice() }) //individ
					setAttributes({ sliderestimates_source: '' }) //individ					
				...
				getToken(raw).then(res => {
					...
					getUserDomains(accessToken).then(res => {
						...
						setAttributes({ sliderestimates_source_arr: existing_sources.slice() }) ////individ
			...
			if (!sliderestimates_source_arr && window.existing_sources) {
				setAttributes({ sliderestimates_source_arr: window.existing_sources }) ////individ
			}
	...
	function setSliderestimatesSource(source, domain_first = false) {
		setAttributes({ sliderestimates_source: source });
		if (domain_first) {
		...
		getProfiles(domain_first).then(
			res => {
				...
				return getDataSliderestimatesWidget(send_data)
			}
		).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ sliderestimates_source_code: final_code });
	...
	function setSliderestimatesSourceCode(code) { //extra capobility
		setAttributes({ sliderestimates_source_code: code });
	}

	function setTitle(title) {
		setAttributes({ sliderestimates_title: title });
		let send_data = {
			sliderestimates_title: title,
			sliderestimates_source: sliderestimates_source,
			profile: profile
		}
		getDataSliderestimatesWidget(send_data).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ sliderestimates_source_code: final_code });
	...
	return (
		<div {...useBlockProps()}>
			<div class="block-widget">
				<div class="block-widget-title">Slider Estimates Widget</div>
				<div class="block-widget-body">
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							onChange={(source) => { setSliderestimatesSource(source) }}
