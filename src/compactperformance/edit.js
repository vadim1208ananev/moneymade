...
import { getDataCompactPerformanceWidget, getTickerBySymbol, getUserData, getToken, getUserDomains, getProfiles } from "./api"
/**
...
export default function Edit(props) {
	const { attributes: { domain, domain_arr, profile, profile_arr, symbol_search, compactperformance_source, auth_result, compactperformance_source_arr, compactperformance_monitization_type, compactperformance_source_code, compactperformance_ticker,
		compactperformance_comparasion_ticker, html_to_iframe
	}, setAttributes } = props;
		...
		if (!window.step_domains) {
			getUserData().then(res => {
				if (!res.status || !res.user_data || !res.prepare_data) {
					...
					setAttributes({ compactperformance_source_arr: empty_sources.slice() }) //individ
					setAttributes({ compactperformance_source: '' }) //individ					
				...
				getToken(raw).then(res => {
					...
					getUserDomains(accessToken).then(res => {
						...
						setAttributes({ compactperformance_source_arr: existing_sources.slice() }) ////individ
			...
			if (!compactperformance_source_arr && window.existing_sources) {
				setAttributes({ compactperformance_source_arr: window.existing_sources }) ////individ
			}
	...
	function setCompactPerformanceSource(source, domain_first = false) {

		setAttributes({ compactperformance_source: source });
		if (domain_first) {
	...
	function setCompactPerformanceMonitizationType(type) {
		...
		getDataCompactPerformanceWidget(send_data).then(
			res => {
	...
	function setCompactPerformanceSourceCode(code) { //extra capobility
		setAttributes({ compactperformance_source_code: code });
	}
	...
	function setTicker(ticker) {
		if (ticker.includes('abbr') && regSticker(ticker)) {
			setAttributes({ compactperformance_ticker: ticker });
			let send_data = {
				compactperformance_monitization_type: compactperformance_monitization_type,
				compactperformance_source: compactperformance_source,
	...
	function setComparasionTicker(ticker) {
		if (ticker.includes('abbr') && regSticker(ticker)) {
			setAttributes({ compactperformance_comparasion_ticker: ticker });
			let send_data = {
				compactperformance_monitization_type: compactperformance_monitization_type,
				compactperformance_source: compactperformance_source,
				compactperformance_ticker: regSticker(compactperformance_ticker),
	...
	function delTicker() {
		setAttributes({ compactperformance_ticker: '' });
		let send_data = {
			compactperformance_monitization_type: compactperformance_monitization_type,
			compactperformance_source: compactperformance_source,
	...
	function delComparasionTicker() {
		setAttributes({ compactperformance_comparasion_ticker: '' });
		let send_data = {
			compactperformance_monitization_type: compactperformance_monitization_type,
			compactperformance_source: compactperformance_source,
	...
	return (
		<div {...useBlockProps()}>
			<div class="block-widget">
				<div class="block-widget-title">Compact Performance Widget</div>
				...
				<div class="block-widget-body">
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							onChange={(source) => { setCompactPerformanceSource(source) }}
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							onChange={(type) => { setCompactPerformanceMonitizationType(type) }}
							options={[
