...
import { getDataСrdkWidget, getTickerBySymbol, getUserData, getToken, getUserDomains, getProfiles } from "./api"
/**
...
export default function Edit(props) {
	const { attributes: { domain, domain_arr, profile, profile_arr, crdk_source, auth_result, crdk_source_arr, crdk_source_code,
		html_to_iframe, crdk_title
	}, setAttributes } = props;
		...
		if (!window.step_domains) {
			getUserData().then(res => {
				if (!res.status || !res.user_data || !res.prepare_data) {
					...
					setAttributes({ crdk_source_arr: empty_sources.slice() }) //individ
					setAttributes({ crdk_source: '' }) //individ					
				...
				getToken(raw).then(res => {
					...
					getUserDomains(accessToken).then(res => {
						...
						setAttributes({ crdk_source_arr: existing_sources.slice() }) ////individ
			...
			if (!crdk_source_arr && window.existing_sources) {
				setAttributes({ crdk_source_arr: window.existing_sources }) ////individ
			}
	...
	function setСrdkSource(source, domain_first = false) {
		setAttributes({ crdk_source: source });
		if (domain_first) {
		...
		getProfiles(domain_first).then(
			res => {
				...
				return getDataСrdkWidget(send_data)
			}
		).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ crdk_source_code: final_code });
				setAttributes({ html_to_iframe: html_to_iframe })
			}
		)
	}

	function setCrdkSourceCode(code) { //extra capobility
		setAttributes({ crdk_source_code: code });
	}

	function setTitle(title) {
		setAttributes({ crdk_title: title });
		let send_data = {
			crdk_title: title,
			crdk_source: crdk_source,
			profile: profile
		}
		getDataСrdkWidget(send_data).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ crdk_source_code: final_code });
	...
	return (
		<div {...useBlockProps()}>
			<div class="block-widget">
				<div class="block-widget-title">Compact Revenue Dark Widget</div>
				<div class="block-widget-body">
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							value={crdk_source}
							onChange={(source) => { setСrdkSource(source) }}
