...
import { getDataTrdWidget, getTickerBySymbol, getUserData, getToken, getUserDomains, getProfiles } from "./api"
/**
...
export default function Edit(props) {
	const { attributes: { domain, domain_arr, profile, profile_arr, trd_source, auth_result, trd_source_arr, trd_source_code,
		html_to_iframe, trd_title
	}, setAttributes } = props;
		...
		if (!window.step_domains) {
			getUserData().then(res => {
				if (!res.status || !res.user_data || !res.prepare_data) {
					...
					setAttributes({ trd_source_arr: empty_sources.slice() }) //individ
					setAttributes({ trd_source: '' }) //individ					
				...
				getToken(raw).then(res => {
					...
					getUserDomains(accessToken).then(res => {
						...
						setAttributes({ trd_source_arr: existing_sources.slice() }) ////individ
			...
			if (!trd_source_arr && window.existing_sources) {
				setAttributes({ trd_source_arr: window.existing_sources }) ////individ
			}
	...
	function setTrdSource(source, domain_first = false) {
		setAttributes({ trd_source: source });
		if (domain_first) {
		...
		getProfiles(domain_first).then(
			res => {
				...
				return getDataTrdWidget(send_data)
			}
		).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ trd_source_code: final_code });
	...
	function setTrdSourceCode(code) { //extra capobility
		setAttributes({ trd_source_code: code });
	}

	function setTitle(title) {
		setAttributes({ trd_title: title });
		let send_data = {
			trd_title: title,
			trd_source: trd_source,
			profile: profile
		}
		getDataTrdWidget(send_data).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ trd_source_code: final_code });
	...
	return (
		<div {...useBlockProps()}>
			<div class="block-widget">
				<div class="block-widget-title">The Revenue Driver Widget</div>
				<div class="block-widget-description">The Revenue Driver is one of our most popular widgets. This Revenue Driver showcases our premium partners with necessary information and data to drive valuable clicks. Our Adtech AI will optimize what products are shown to your users in every widget, even on specific articles - all based on the highest RPMs.</div>
				...
				<div class="block-widget-body">
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							value={trd_source}
							onChange={(source) => { setTrdSource(source) }}
