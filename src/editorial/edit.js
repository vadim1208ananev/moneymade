...
import { getDataEditorialWidget, getTickerBySymbol, getUserData, getToken, getUserDomains, getProfiles } from "./api"
/**
...
export default function Edit(props) {
	const { attributes: { domain, domain_arr, profile, profile_arr, editorial_source, auth_result, editorial_source_arr, editorial_source_code,
		html_to_iframe, editorial_title
	}, setAttributes } = props;
		...
		if (!window.step_domains) {
			...
			getUserData().then(res => {
				if (!res.status || !res.user_data || !res.prepare_data) {
					...
					setAttributes({ editorial_source_arr: empty_sources.slice() }) //individ
					setAttributes({ editorial_source: '' }) //individ					
				...
				getToken(raw).then(res => {
					...
					getUserDomains(accessToken).then(res => {
						...
						setAttributes({ editorial_source_arr: existing_sources.slice() }) ////individ
			...
			if (!editorial_source_arr && window.existing_sources) {
				setAttributes({ editorial_source_arr: window.existing_sources }) ////individ
			}
	...
	function setEditorialSource(source, domain_first = false) {
		setAttributes({ editorial_source: source });
		if (domain_first) {
		...
		getProfiles(domain_first).then(
			res => {
				...
				return getDataEditorialWidget(send_data)
			}
		).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ editorial_source_code: final_code });
				setAttributes({ html_to_iframe: html_to_iframe })
			}
		)
	}

	function setEditorialSourceCode(code) { //extra capobility
		setAttributes({ editorial_source_code: code });
	}

	function setTitle(title) {
		setAttributes({ editorial_title: title });
		let send_data = {
			editorial_title: title,
			editorial_source: editorial_source,
			profile: profile
		}
		getDataEditorialWidget(send_data).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ editorial_source_code: final_code });
	...
	return (
		<div {...useBlockProps()}>
			<div class="block-widget">
				<div class="block-widget-title">Editorial Widget</div>
				<div class="block-widget-description">This widget has a more native content look and feel.</div>
				...
				<div class="block-widget-body">
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							onChange={(source) => { setEditorialSource(source) }}
