...
import { getDataTopnftWidget, getTickerBySymbol, getUserData, getToken, getUserDomains, getProfiles } from "./api"
/**
...
export default function Edit(props) {
	const { attributes: { domain, domain_arr, profile, profile_arr, topnft_source, auth_result, topnft_source_arr, topnft_monitization_type, topnft_source_code,
		html_to_iframe, topnft_title
	}, setAttributes } = props;
		...
		if (!window.step_domains) {
			...
			getUserData().then(res => {
				if (!res.status || !res.user_data || !res.prepare_data) {
					...
					setAttributes({ topnft_source_arr: empty_sources.slice() }) //individ
					setAttributes({ topnft_source: '' }) //individ					
				...
				getToken(raw).then(res => {
					...
					getUserDomains(accessToken).then(res => {
						...
						setAttributes({ topnft_source_arr: existing_sources.slice() }) ////individ
			...
			if (!topnft_source_arr && window.existing_sources) {
				setAttributes({ topnft_source_arr: window.existing_sources }) ////individ
			}
	...
	function setTopnftSource(source, domain_first = false) {
		setAttributes({ topnft_source: source });
		if (domain_first) {
		...
		getProfiles(domain_first).then(
			res => {
				...
				return getDataTopnftWidget(send_data)
			}
		).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ topnft_source_code: final_code });
	...
	function setProfile(item) {
		...
		getDataTopnftWidget(send_data).then(
			res => {
				let final_code = res.final_code
				let html_to_iframe = res.html_to_iframe
				setAttributes({ topnft_source_code: final_code });
	...
	function setTopnftMonitizationType(type) {
		setAttributes({ topnft_monitization_type: type });
		let send_data = {
			topnft_source: topnft_source,
	...
	function setTopnftSourceCode(code) { //extra capobility
		setAttributes({ topnft_source_code: code });
	}

	function setTitle(title) {
		setAttributes({ topnft_title: title });
		let send_data = {
			topnft_title: title,
			topnft_source: topnft_source,
			topnft_monitization_type: topnft_monitization_type,
	...
	return (
		<div {...useBlockProps()}>
			<div class="block-widget">
				<div class="block-widget-title">Top NFTs Widget</div>
				<div class="block-widget-description">Widget which displays most popular collections/sales/marketplaces</div>
				...
				<div class="block-widget-body">
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							value={topnft_source}
							onChange={(source) => { setTopnftSource(source) }}
					...
					<div class="block-widget-body-elem">
						<SelectControl
							...
							onChange={(type) => { setTopnftMonitizationType(type) }}
							options={[
